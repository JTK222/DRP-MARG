...
public class TextureEditors {
	 ...
	 * @param sourceImage
	 * @param overlayImage
	 * @return a modified copy of the sourceImage, overlayed with the overlayImage
	 */
	...
	public static BufferedImage rotateImage(BufferedImage sourceImage, int rotationAngle) {
		double sin = Math.abs(Math.sin(Math.toRadians(rotationAngle))),
				cos = Math.abs(Math.cos(Math.toRadians(rotationAngle)));
	 ...
	 * @param sourceImage
	 * @param flipOnX if true the sourceImage is flipped horizontally, otherwise vertically
	 * @return a modified copy of the sourceImage
	 */
